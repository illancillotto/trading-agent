...
export function MarketData({ symbols = ['BTC'] }: { symbols?: string[] }) {
    ...
    const fetchData = async () => {
        setLoading(true)
        try {
            const res = await fetch(`${API_BASE_URL}/api/market-data/aggregate?symbol=${selectedSymbol}`)
    ...
    // Aggiorna selectedSymbol se la lista symbols cambia
    useEffect(() => {
    ...
    }, [selectedSymbol]) // Ricarica quando cambia il simbolo selezionato
    ...
    const avgPrice = global_market?.average_price
    const spread = global_market?.price_spread_pct
    const funding = global_market?.average_funding_rate
    const deviation = global_market?.hyperliquid_deviation_pct
        ...
        <div className="bg-white border rounded-lg p-4 shadow-sm">
            <div className="flex justify-between items-center mb-4">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <div className="flex gap-1 mr-2">
                        {symbols.map(sym => (
                            <button
                                ...
                                className={`px-2 py-1 text-xs font-medium rounded ${selectedSymbol === sym
                    ...
                    <span className="text-xs text-gray-400 hidden sm:inline">Sources: {global_market?.sources_count || 0}</span>
                    <button
                        onClick={fetchData}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                <div className="p-3 bg-gray-50 rounded border border-gray-100">
                    ...
                    <div className="font-mono font-bold text-gray-900">${safeToLocaleString(avgPrice, { maximumFractionDigits: 2 })}</div>
                </div>

                <div className="p-3 bg-gray-50 rounded border border-gray-100">
                    <div className="text-xs text-gray-500 uppercase mb-1">Spread</div>
                    <div className="font-mono font-bold text-gray-900">{safeToFixed(spread, 3)}%</div>
                </div>

                <div className="p-3 bg-gray-50 rounded border border-gray-100">
                    <div className="text-xs text-gray-500 uppercase mb-1">Avg Funding</div>
                    <div className={`font-mono font-bold ${(funding || 0) > 0 ? 'text-green-600' : 'text-red-600'}`}>
                        {safeToFixed(funding, 6)}%
                    </div>
                </div>

                <div className="p-3 bg-gray-50 rounded border border-gray-100">
                    <div className="text-xs text-gray-500 uppercase mb-1">HL Deviation</div>
                    <div className={`font-mono font-bold ${(deviation || 0) > 0 ? 'text-amber-600' : 'text-blue-600'
                        }`}>
                        {deviation !== null && deviation !== undefined ? `${deviation > 0 ? '+' : ''}${deviation.toFixed(4)}%` : '-'}
                    </div>
                </div>
            </div>
        </div>
